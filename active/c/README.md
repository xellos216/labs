...
# Phase 00 — Assembly Primer for C/GDB
## Goal
Build the minimum assembly-level mental model needed to understand C execution, GDB output and stack behavior.
This phase is not a full assembly course.
It exists as a bridge between basic C knowledge and the later phases on memory, ELF files, GDB and x86-64 assembly.
...
- explain that the CPU executes instructions step by step
- understand registers as small CPU-side storage locations
- recognize basic data movement and arithmetic instructions
- distinguish instruction flow from C source-code flow
- explain the basic idea of stack usage during function calls
- understand `call`, `ret`, `rip`, `rsp`, `rbp` at a beginner level
- connect simple C functions to basic GDB and disassembly output
- reduce confusion when observing stack frames and saved return addresses
...
## Role of Pocket Assembler
Pocket Assembler is used only as a warm-up tool.
Its purpose is to build intuition for:
