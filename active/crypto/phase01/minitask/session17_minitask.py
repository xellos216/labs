...
is_hex = all(c in hex_str for c in token.decode())
is_base64 = all(c in base64_str for c in token.decode())
peel = base64.urlsafe_b64decode(token)
parts = peel.decode().split(".")
