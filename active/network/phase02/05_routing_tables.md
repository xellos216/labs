...
For local-link destinations such as `192.168.0.1` or `203.0.113.254`, Linux uses the directly connected network route, not the default route.
...
```text
default via 203.0.113.254 dev eth0 proto dhcp src 203.0.113.64 metric 100
default via 192.168.0.1 dev wlan0 proto dhcp src 192.168.0.104 metric 600
```
...
but the reason wlan0 is selected is not because Linux is using the default route.
...
```text
203.0.113.0/24 dev eth0 proto kernel scope link src 203.0.113.64 metric 100
```
...
MAC = aa:bb:cc:dd:ee:ff
        ↓
Ethernet frame sent through eth0
```
...
### Q4. Why does `192.168.0.1` use `wlan0` instead of the lower-metric `eth0`?
